...
g, steps, prob_scatter = 0.999, 16, 0.5
turb = 1.0
...
mie_coef = (0.6544*turb - 0.6510)*10**(-16)
beta_mie, beta_rayleigh = 0.217*mie_coef*4*(pi**2), (8 / (3 * 2.504e25)) * (pi ** 3) * (n ** 2 - 1) ** 2
...
# log_shell_height = np.linspace(0, altura_atm, steps)
...
def scatter_transmission(h, angle, w):
    ...
    ratio_mie = beta_m * rho_mie * phase_mie
...
def sum_to_opt_dep(r, dis, w, one_step=False):
    ...
    if one_step:
        ...
        opt_dep = 4 * pi * (beta_m * rho_mie + beta_ray * rho_rayleigh) * dis.x
        return opt_dep
    ...
    while num <= n_max:
        ...
        opt_dep += 4 * pi * (beta_m * rho_mie + beta_ray * rho_rayleigh) * step_displace.x
